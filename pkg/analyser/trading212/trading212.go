...
func (a Trading212) GetAllCandidateAccounts(cfg *config.Config) map[string]bool {
	// uniqMap will be used to create the concepts.
	uniqMap := make(map[string]bool)

	if cfg.Trading212 == nil || len(cfg.Trading212.Rules) == 0 {
		return uniqMap
	}

	for _, r := range cfg.Trading212.Rules {
...
func (a Trading212) GetAccountsAndTags(o *ir.Order, cfg *config.Config, target, provider string) (bool, string, string, map[ir.Account]string, []string) {
	ignore := false
	...
	for _, r := range cfg.Trading212.Rules {
		...
		if r.Note != nil {
			match = matchFunc(*r.Note, o.Note, sep, match)
		}
